#!/usr/bin/env python3
"""Aggregate BDLO evaluation results across both folders, excluding corrupted data."""

import csv
import numpy as np
from pathlib import Path

# Base directories for BDLO evaluation results
BASE_DIRS = [
    Path(__file__).parent / 'bdlo1_evaluation_results',
    Path(__file__).parent / 'bdlo1_faster_evaluation_results',
]

# Corrupted clips to exclude (in bdlo1_faster_evaluation_results only)
CORRUPTED_FASTER = {
    ('chunk_0', 'clip_0'),
    ('chunk_0', 'clip_1'),
    ('chunk_6', 'clip_0'),
...
def main():
    print("Collecting frames from BDLO evaluation results (excluding corrupted data)...")
    ...
    print(f"\n{'='*100}")
    print(f"BDLO RESULTS (merged from both folders, excluding corrupted data)")
