"""
Batch BDLO tracking experiment on bdlo_yellow_2sec dataset.

Processes chunks with multiple clips, reinitializing trackers per clip.
BDLO2 has 2 branch nodes and 5 leaf nodes.
...
Topology:
    ee0 --- b0 --- free_leaf_0
            ...
            b1 --- ee1
           / \
  free_leaf_1_top  free_leaf_1_down
...
Segments (6 total):
    ...
    2: b0 → b1
    3: b1 → ee1
    4: b1 → free_leaf_1_top
    5: b1 → free_leaf_1_down

Default interior keypoints per segment: [1, 4, 6, 6, 2, 2]
Total keypoints: 2 branch + 5 leaf + 21 interior = 28

Usage:
    python bdlo2_batch_experiment.py --chunk 0 --clip_seconds 20
    python bdlo2_batch_experiment.py --chunk 5 --clip_seconds 30
...
def load_chunk_data(chunk_dir: Path) -> dict:
    ...
    masks_path = chunk_dir / 'bdlo_masks' / 'masks.npz'
...
class CDCPDBDLOTracker:
    ...
    def __init__(self, intrinsics, n_keypoints=28, ee_poses_3d=None, **kwargs):
...
def create_ablation_grid(panels, frame_idx, shape_hw, method_names=None):
    ...
    cv2.putText(info, "BDLO2 Ablation", (W//4, H//3), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
...
def create_full_tracking_visualization(rgb, depth_augmented_mask, skeleton_mask, keypoints_2d, edges,
    ...
    # Panel 2: Depth-augmented SAM2 mask
    panel2 = rgb.copy()
    ...
    if keypoints_2d is not None and len(keypoints_2d) > 0:
        ...
        # BDLO2 coloring for panel3
        BRANCH_COLOR = (128, 0, 128)  # Purple
        for idx, (row, col) in enumerate(kp_int):
            if 0 <= row < H and 0 <= col < W:
                if idx < 2:  # Branch
                    color = BRANCH_COLOR
                elif idx < 7:  # Leaf (5 leaves)
                    color = LEAF_COLOR
                else:  # Intermediate
                    color = INTER_COLOR
    ...
    cv2.putText(panel4, f"BDLO2: 2 branch, 5 leaf", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
def process_clip(data, transforms, ee_poses_3d, clip_idx, start_frame, end_frame,
                 output_dir, n_keypoints=28, tail_length=60, fps=30, keypoints_per_segment=None):
    ...
    for local_idx, global_idx in enumerate(tqdm(range(start_frame, end_frame), desc=f"    Clip {clip_idx}")):
        ...
        for method in method_names:
            ...
            try:
                ...
                # Save debug image on crash
                if local_idx == 0 and method == 'Full':
                    try:
                        debug_img = rgb.copy()
                        # Overlay depth-filtered foreground mask in green
                        valid_depth = (depth > 0) & (depth < 2000)
                        fg_filtered = (dlo_mask > 0) & valid_depth
                        fg_overlay = np.zeros_like(debug_img)
                        fg_overlay[fg_filtered] = (0, 255, 0)
                        debug_img = cv2.addWeighted(debug_img, 0.7, fg_overlay, 0.3, 0)
                        # Draw identified branch/leaf nodes from tracker
                        if hasattr(tracker, '_last_branch_2d') and tracker._last_branch_2d is not None:
                            for idx, (r, c) in enumerate(tracker._last_branch_2d):
                                r, c = int(r), int(c)
                                cv2.circle(debug_img, (c, r), 8, (128, 0, 128), -1)
                                cv2.putText(debug_img, f"B{idx}", (c+10, r-4),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (128, 0, 128), 2)
                        if hasattr(tracker, '_last_leaf_2d') and tracker._last_leaf_2d is not None:
                            for idx, (r, c) in enumerate(tracker._last_leaf_2d):
                                r, c = int(r), int(c)
                                cv2.circle(debug_img, (c, r), 8, (255, 255, 0), -1)
                                cv2.putText(debug_img, f"L{idx}", (c+10, r-4),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
                        cv2.putText(debug_img, f"Error: {e}", (10, 30),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                        debug_path = clip_output_dir / 'init_debug_crash.png'
                        cv2.imwrite(str(debug_path), cv2.cvtColor(debug_img, cv2.COLOR_RGB2BGR))
                        print(f"  Saved crash debug: {debug_path}")
                    except Exception as e2:
                        print(f"  Failed to save debug image: {e2}")
                raise

            if method == 'Full':
                full_result = result

            # Save debug init image on first frame for Full method
            if local_idx == 0 and method == 'Full':
                debug_img = rgb.copy()
                skel = result.get('skeleton_mask')
                if skel is not None and np.any(skel > 0):
                    skel_thick = cv2.dilate(skel, np.ones((3, 3), np.uint8), iterations=1)
                    debug_img[skel_thick > 0] = (0, 191, 255)
                if result['success']:
                    kp2d = result['keypoints_2d'].astype(int)
                    n_b = tracker.reference_n_branch if hasattr(tracker, 'reference_n_branch') else 2
                    n_l = tracker.reference_n_leaf if hasattr(tracker, 'reference_n_leaf') else 5
                    for idx, (r, c) in enumerate(kp2d):
                        if 0 <= r < rgb.shape[0] and 0 <= c < rgb.shape[1]:
                            if idx < n_b:
                                color, label = (128, 0, 128), f"B{idx}"
                            elif idx < n_b + n_l:
                                color, label = (255, 255, 0), f"L{idx - n_b}"
                            else:
                                color, label = (255, 165, 0), str(idx)
                            cv2.circle(debug_img, (c, r), 6, color, -1)
                            cv2.putText(debug_img, label, (c + 8, r - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                    if result.get('edges'):
                        for (i, j) in result['edges']:
                            if i < len(kp2d) and j < len(kp2d):
                                cv2.line(debug_img, (kp2d[i, 1], kp2d[i, 0]), (kp2d[j, 1], kp2d[j, 0]), (50, 205, 50), 2)
                debug_path = clip_output_dir / 'init_debug.png'
                cv2.imwrite(str(debug_path), cv2.cvtColor(debug_img, cv2.COLOR_RGB2BGR))
                print(f"  Saved init debug: {debug_path}")
            ...
            if result['success']:
                ...
                if local_idx == 0 and method == 'Full':
                    ...
                    if hasattr(tracker, 'segment_edges') and tracker.segment_edges is not None:
                        seg_names = ['ee0->b0', 'b0->free_leaf_0', 'b0->b1', 'b1->ee1', 'b1->free_leaf_1_top', 'b1->free_leaf_1_down']
...
def main():
    parser = argparse.ArgumentParser(description='Batch BDLO2 tracking experiment')
    ...
    parser.add_argument('--n_keypoints', type=int, default=23, help='Number of keypoints (default: 28 for BDLO2)')
    parser.add_argument('--keypoints_per_segment', type=int, nargs=6, default=None,
                        help='Interior keypoints per segment: [ee0->b0, b0->free_leaf_0, b0->b1, b1->ee1, b1->free_leaf_1_top, b1->free_leaf_1_down] (default: [1,4,6,6,2,2])')
    args = parser.parse_args()

    # Default keypoints_per_segment for BDLO2
    if args.keypoints_per_segment is None:
        args.keypoints_per_segment = [1, 6, 4, 2, 2, 6]
        args.keypoints_per_segment = [1, 5, 3, 1, 1, 5]
        # args.keypoints_per_segment = [0, 2, 3, 1, 1, 2]

    # Compute n_keypoints from keypoints_per_segment
    n_keypoints = args.n_keypoints
    # BDLO2: 2 branch + 5 leaf + sum(interior)
    computed_n = 2 + 5 + sum(args.keypoints_per_segment)
    ...
    # Paths
    data_base = Path('/mnt/mydisk/captured_data_double_arm/bdlo_yellow_2sec')
    ...
    output_base = Path('./bdlo2_evaluation_results')
    ...
    print("=" * 80)
    print(f"BDLO2 BATCH EXPERIMENT - Chunk {args.chunk}")
    print("=" * 80)
    ...
    # Compute chunk aggregate summary (Frame-weighted)
    ...
    # Compute chunk aggregate summary (Clip-weighted)
