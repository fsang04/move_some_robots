"""
BDLO Faster Tracking Projection Ablation Study
...
Dataset: bdlo_no_contact_2sec (faster motion variant)
...
Usage:
    python bdlo_faster_tracking_ablation.py --chunk 0 --clip_seconds 20
...
OUTPUT_BASE = Path('./bdlo_faster_tracking_ablation_results')
...
class BDLOFasterTrackerNoProj(WireTracker):
    """BDLO Faster tracker with projection disabled during geometry constraint."""
...
def process_clip(data, transforms, ee_poses_3d, start_frame, end_frame, clip_idx,
    ...
    # Construct intrinsics matrix explicitly (same as bdlo1_faster_batch_experiment.py)
    K = transforms['K']
    intrinsics = np.array([
        [K[0, 0], 0, K[0, 2]],
        [0, K[1, 1], K[1, 2]],
    ...
    trackers = {
        ...
        'NoProj': BDLOFasterTrackerNoProj(**base_params),
    }
...
def aggregate_chunk_summary(output_dir, all_clip_results, method_names):
    ...
    with open(summary_path, 'w') as f:
        f.write("BDLO Faster Tracking Projection Ablation - Chunk Aggregate Summary\n")
        f.write("=" * 100 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='BDLO Faster Tracking Projection Ablation')
    ...
    print("=" * 80)
    print(f"BDLO FASTER TRACKING PROJECTION ABLATION - Chunk {args.chunk}")
    print("=" * 80)
