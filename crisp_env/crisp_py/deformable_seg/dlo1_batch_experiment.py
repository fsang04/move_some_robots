"""
Batch DLO tracking experiment on dlo1_first400 dataset.

Processes chunks with multiple clips, reinitializing trackers per clip.

Usage:
    python dlo1_batch_experiment.py --chunk 0 --clip_seconds 20
    python dlo1_batch_experiment.py --chunk 5 --clip_seconds 30
...
class CDCPDDLOTracker:
    ...
    def process_frame(self, depth, arm_depth=None, rgb=None, precomputed_arm_mask=None):
        ...
        # No anchors — let CDCPD run freely
        anchor_indices = None
...
def create_full_tracking_visualization(rgb, foreground_mask, skeleton_mask, keypoints_2d, edges,
    ...
    cv2.putText(panel4, f"Chain: 0(L)->...->9(R)", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
def process_clip(data, transforms, ee_poses_3d, clip_idx, start_frame, end_frame,
                 output_dir, n_keypoints=10, tail_length=60, fps=30):
    ...
    for local_idx, global_idx in enumerate(tqdm(range(start_frame, end_frame), desc=f"    Clip {clip_idx}")):
        ...
        if full_result is not None:
            ...
            full_skeleton_mask = full_result.get('clean_path_mask',
            ...
            full_vis = create_full_tracking_visualization(
                rgb=rgb, foreground_mask=dlo_mask, skeleton_mask=full_skeleton_mask,
...
def main():
    parser = argparse.ArgumentParser(description='Batch DLO tracking experiment')
    ...
    # Paths
    data_base = Path('/mnt/mydisk/captured_data_double_arm/dlo_blue_4sec')
    ...
    output_base = Path('./dlo_blue_4sec_evaluation_results_correct_ee')
    ...
    print("=" * 80)
    print(f"DLO BATCH EXPERIMENT - Chunk {args.chunk}")
    print("=" * 80)
