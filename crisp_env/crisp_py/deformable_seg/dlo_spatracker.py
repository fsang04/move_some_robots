...
def process_clip(
    ...
    for seg_idx in range(n_segments):
        ...
        for local_frame in range(seg_len):
            ...
            n_samples = max(10, n_gt_points // N_EDGES)
...
def main():
    ...
    # Validate chunk
    if not 0 <= args.chunk <= 19:
        print(f"ERROR: Chunk {args.chunk} not in valid range 0-19")
        return
    ...
    print("=" * 80)
    print(f"SPATRACKERV2 DLO TRACKING - Chunk {args.chunk}")
    print("=" * 80)
    ...
    for clip_idx in range(n_clips):
        ...
        # Check if evaluation results exist (DLO uses clip_0, clip_1 format)
    ...
    if all_results:
        ...
        with open(chunk_summary_path, 'w') as f:
            f.write(f"SpaTrackerV2 DLO Tracking - Chunk {args.chunk}\n")
            f.write("=" * 100 + "\n\n")
