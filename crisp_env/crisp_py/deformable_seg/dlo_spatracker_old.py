#!/usr/bin/env python3
"""
SpaTrackerV2 baseline for DLO (Deformable Linear Object) tracking experiment.

Compares SpaTrackerV2 against our WireTracker on dlo1_first400 dataset.
Static camera, RGBD input (BGR color + mm depth).

Dataset: dlo1_first400 (chunks 0-19, clip duration 15s at 30fps = 450 frames)

Usage:
    python dlo_spatracker.py --chunk 0
    python dlo_spatracker.py --chunk 0 --clip_seconds 15
    python dlo_spatracker.py --chunk 0 --segment_seconds 5
...
EVAL_RESULTS_BASE = Path('/home/roahmlab/move_some_robots/crisp_env/crisp_py/deformable_seg/dlo1_evaluation_results')

# Output base path
OUTPUT_BASE = Path('/home/roahmlab/move_some_robots/crisp_env/crisp_py/deformable_seg/dlo_spatracker_results')

# DLO has 14 keypoints in a line
N_KEYPOINTS = 14
N_EDGES = 13  # consecutive connections
...
FPS = 30
DEFAULT_CLIP_SECONDS = 15  # 450 frames
...
def load_chunk_data(chunk_dir: Path, max_frames: int = 10000) -> dict:
    ...
    # Load DLO mask from masks/masks.npz
    mask_path = chunk_dir / 'masks' / 'masks.npz'
    if mask_path.exists():
        dlo_mask = np.load(mask_path)['masks'][start_idx:]
        result['dlo_mask'] = dlo_mask > 0
        print(f"    DLO mask: {dlo_mask.shape}")
    else:
...
def save_tracking_video(
    ...
    Args:
        mask: (T, H, W) bool foreground mask - if provided, overlay 50% red on foreground
    """
    ...
    for t in tqdm(range(T), desc="Creating video", leave=False):
        ...
        rgb = video_frames[t][..., ::-1].copy()
        
        # Apply 50% red overlay on foreground mask
        if mask is not None and t < len(mask):
            fg = mask[t]
            # Blend with red: output = 0.5 * original + 0.5 * red
            rgb[fg, 0] = (rgb[fg, 0].astype(np.float32) * 0.5 + 255 * 0.5).astype(np.uint8)  # R
            rgb[fg, 1] = (rgb[fg, 1].astype(np.float32) * 0.5).astype(np.uint8)  # G
            rgb[fg, 2] = (rgb[fg, 2].astype(np.float32) * 0.5).astype(np.uint8)  # B
...
def process_clip(
    ...
    print(f"    Keypoints: {keypoints_3d.shape[0]} (linear chain)")
    ...
    for seg_idx in range(n_segments):
        ...
        for local_frame in range(seg_len):
            ...
            n_gt_points = len(point_cloud) if point_cloud is not None else 200
            n_samples_per_edge = max(5, n_gt_points // len(edge_connections)) if len(edge_connections) > 0 else 10
            pred_cloud = sample_points_on_edges(kp_3d, edge_connections, n_samples_per_edge=n_samples_per_edge)
    ...
    np.savez(
        ...
        edge_connection=edge_connections,
    ...
    with open(summary_path, 'w') as f:
        f.write(f"SpaTrackerV2 DLO Tracking - Clip {clip_idx}\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"Frames: {n_frames}\n")
        f.write(f"Keypoints: {N_KEYPOINTS} (linear chain)\n")
...
def main():
    parser = argparse.ArgumentParser(description='SpaTrackerV2 baseline for DLO tracking')
    parser.add_argument('--chunk', type=int, required=True, help='Chunk index (0-19)')
    parser.add_argument('--clip_seconds', type=int, default=DEFAULT_CLIP_SECONDS,
                        help=f'Clip duration in seconds (default: {DEFAULT_CLIP_SECONDS})')
    ...
    print("=" * 80)
    print(f"SPATRACKERV2 DLO TRACKING - dlo1_first400 Chunk {args.chunk}")
    print("=" * 80)
    ...
    if all_results:
        ...
        with open(chunk_summary_path, 'w') as f:
            f.write(f"SpaTrackerV2 DLO Tracking - dlo1_first400 Chunk {args.chunk}\n")
            f.write("=" * 100 + "\n\n")
