"""
DLO Tracking Projection Ablation Study
...
Dataset: dlo1_first400 (same as dlo1_batch_experiment.py)
...
Usage:
    python dlo_tracking_ablation.py --chunk 0 --clip_seconds 20
...
OUTPUT_BASE = Path('./dlo_tracking_ablation_results')
...
class DLOTrackerNoProj(WireTracker):
    """DLO tracker with projection disabled during geometry constraint."""
    ...
    def _dlo_two_stage_balanced_optimization(
        ...
        """
        DLO geometry constraint with projection DISABLED.
        Only applies sequential edge correction (no projection blend).
        """
        ...
        # Just do edge correction, no projection
        for _ in range(self.n_edge_iterations):
...
def process_clip(data, transforms, ee_poses_3d, start_frame, end_frame, clip_idx,
                 output_dir, n_keypoints, fps, tail_length=60):
    ...
    # Construct intrinsics matrix explicitly (same as dlo1_batch_experiment.py)
    K = transforms['K']
    intrinsics = np.array([
        [K[0, 0], 0, K[0, 2]],
        [0, K[1, 1], K[1, 2]],
    ...
    # Must match dlo1_batch_experiment.py parameters
    base_params = {
    ...
    trackers = {
        ...
        'NoProj': DLOTrackerNoProj(**base_params),
    }
...
def aggregate_chunk_summary(output_dir, all_clip_results, method_names):
    ...
    for clip_result in all_clip_results:
        ...
        # Load 3d_keypoints from each clip
        ...
        if kpts_path.exists():
            ...
            if 'reference_lengths' in kpts_data and len(all_reference_lengths) == 0:
                all_reference_lengths = kpts_data['reference_lengths']
    ...
    with open(summary_path, 'w') as f:
        f.write("DLO Tracking Projection Ablation - Chunk Aggregate Summary\n")
        f.write("=" * 100 + "\n\n")
        ...
        for method in method_names:
            ...
            # Skip first frame of each clip for averaging
...
def main():
    parser = argparse.ArgumentParser(description='DLO Tracking Projection Ablation')
    ...
    print("=" * 80)
    print(f"DLO TRACKING PROJECTION ABLATION - Chunk {args.chunk}")
    print("=" * 80)
