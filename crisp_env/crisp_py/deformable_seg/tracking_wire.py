...
if __name__ == "__main__":
    ...
    # Save original pc_mask before skeletonization for visualization
    pc_mask_original = pc_mask.copy()
    ...
    gmm_visual_path = "/home/yehengz/deformable_seg/tracking_output/point_cloud_gmm_constrained_wires_tracking.html"
    ...
    mask_save_path = "/home/yehengz/deformable_seg/tracking_output/pc_mask_skeleton_tracking.png"
    create_skel_mask_viz(
        rgb_image,
        pc_mask_original,  # Use original mask before skeletonization
        skeleton_pc_mask,
        branch_nodes,
        end_nodes,
        save_path=mask_save_path,
        skeleton_color=(255, 0, 0),  # Red for skeleton
    )
    print(f"Saved 2D mask visualization: {mask_save_path}")

    # --- Project 3D keypoints to 2D, overlay, and print coordinates ---
    ...
    print("Projected 2D keypoint pixel coordinates:")
    for idx, (row, col) in enumerate(keypoints_2d_int):
        print(f"Keypoint {idx}: (row={row}, col={col})")

    # Overlay 2D keypoints and edges on the image (all in red)
    import matplotlib.pyplot as plt
    from matplotlib.patches import Circle
    img_vis = rgb_image.copy()
    fig, ax = plt.subplots(figsize=(10, 8))
    ax.imshow(img_vis)
    # Draw edges (connections) in red
    for i, j in wire_edges:
        pt1 = keypoints_2d_int[i]
        pt2 = keypoints_2d_int[j]
    ...
    for (row, col) in keypoints_2d_int:
        ...
        ax.add_patch(circ)
    ax.set_title("2D Projection of Keypoints and Connections (Red)")
    ax.axis('off')
    plt.tight_layout()
    plt.savefig("/home/yehengz/deformable_seg/tracking_output/keypoints_2d_overlay.png", dpi=150)
    plt.show()


    print("Saved 2D keypoints overlay: /home/yehengz/deformable_seg/tracking_output/keypoints_2d_overlay.png")

    # Print connection map between keypoints
    print("\nConnection map between keypoints (edges):")
    for idx, (i, j) in enumerate(wire_edges):
        print(f"Edge {idx}: Keypoint {i} <-> Keypoint {j}")
