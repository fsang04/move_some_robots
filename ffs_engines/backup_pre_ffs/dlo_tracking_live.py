...
class ChunkRecorder:
    ...
    def save(self, K, depth_offset_px=0.0):
        if not self.color:
            return
        (self.out_dir / 'masks').mkdir(parents=True, exist_ok=True)
        (self.out_dir / 'calibration').mkdir(parents=True, exist_ok=True)
        # depth_offset_px is provenance, not data: it records the disparity
        # correction ALREADY APPLIED to this depth, so a solver re-run on this
        # session does not apply it a second time (the guard zed_depth_config
        # .dataset_applied_offset_px reads).
...
def main():
    ...
    if recorder is not None:
        recorder.save(K, depth_offset_px=getattr(source, 'depth_offset_px', None) or 0.0)
